...
class App(customtkinter.CTk):
    ...
    def btn_pedir_clave_on_click(self):
        password = "utn750"
        dato_ingresado = prompt(title="Password", prompt="ingresa la contraseña, acordate q es utn750")
        while (dato_ingresado != password):
            alert(title="validacion", message="ingresa una contraseña correcta")
            dato_ingresado = prompt(title="Password", prompt="ingresa la contraseña, acordate q es utn750")
            
        alert(title="password", message="saliste")
