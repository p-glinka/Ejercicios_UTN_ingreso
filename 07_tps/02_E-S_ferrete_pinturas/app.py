...
class App(customtkinter.CTk):
    ...
    def btn_convertir_c_f_on_click(self):
        ...
        temperatura_c_int = int(temperatura_c_str)
        paso_a_f = (temperatura_c_int * 9 / 5 ) + 32
        
        mensaje_concatenado = "({0} °C x 9 / 5) + 32 = {1} ".format(temperatura_c_int,paso_a_f)
        alert(title="Paso de °C a F", message=mensaje_concatenado)

    def btn_convertir_f_c_on_click(self):
        temperatura_f_str = self.txt_temperatura_f.get()
        temperatura_f_int = int(temperatura_f_str)
        paso_a_c = (temperatura_f_int - 32 ) * 5 / 9
        
        mensaje_concatenado = "({0} °C x 9 / 5) + 32 = {1} ".format(temperatura_f_int,paso_a_c)
        alert(title="Paso de F a °C ", message=mensaje_concatenado)
